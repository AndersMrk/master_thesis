...
Consider a pipe with $n$ leaks, where the length of the pipe, $\ell$, and the resistance coefficient, $\theta$, are assumed to be known. The goal is to identify the set of unknown leak parameters $\{z_i,c_i\},\;i\in\{1,2,\dots,n\}$ given a set of boundary measurements $\mathcal{M}\in\mathbb{R}^{4 \times m}$. The leak positions $z_i$ are uniquely defined by the length of the non-leaking pipe sections $L_i$ by 
\begin{equation*}
    z_i=\sum_{k=1}^i L_k.
\end{equation*}
Let
...
To identify all leak parameters, the number of measurements must be sufficient relative to the number of unknown variables. From \cref{eq:main_function}, each steady state operating point, $m$, provides two scalar equations for outlet head and outlet flow. The parameter vector
\[
\mathbf{x} = [L_1,\dots,L_n,c_1,\dots,c_n] \in \mathbb{R}^{2n}
\]
contains $2n$ unknowns, so the measurements must satisfy
\begin{equation*}
    m \geq n,
\end{equation*}
assuming each measurement provides independent information. Under this assumption, the nonlinear system
\[
r(\mathbf{x}) = F(\mathbf{x},\mathcal{M}_0) - \mathcal{M}_L = 0
\]
contains at least as many independent equations as unknowns.\\

If $n$, $\theta$, and $\ell$ are known,and the measurement set $\mathcal{M}$ provides sufficiently independent equations, a solution $\mathbf{x}^*$ satisfying
\[
r(\mathbf{x}^*) = 0
\]
reproduces all measured inlet–outlet relations across the operating points considered. The solution $\mathbf{x}^*$ is then a valid solution to the inverse problem defined in \cref{eq:inverse_problem_def}.\\

A solution to $r(\mathbf{x}) = 0$ does not guarantee that $\mathbf{x}^*$ corresponds to the true physical leak configuration. The nonlinear forward model may admit multiple solutions that produce identical boundary measurements. As discussed in \autoref{sec:sol_existence}, local uniqueness depends on properties of the Jacobian, while global uniqueness requires additional structural assumptions on the forward operator. \\

Even when $m \geq n$ and a solution is found, there is no guarantee for uniqueness. Multiple leak configurations may be consistent with the same measurement set. A rigorous analysis of existence and uniqueness would require a detailed structural study of the forward operator and its Jacobian, which lies outside the scope of this work. Instead, the method is evaluated empirically through simulation experiments in \autoref{chap:results_performance}. The results show that accurate leak configurations are recovered across a wide range of test cases, which supports the practical applicability of the approach. \autoref{chap:analysis_of_sol} shows how the choice of measurement set affects the conditioning of the Jacobian and indicates when reliable solutions can be expected. Together, these results provide practical evidence of feasibility, even though formal guarantees are not established.
