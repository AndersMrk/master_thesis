...
\begin{description}[itemsep=2pt, topsep=0pt, parsep=0pt]
    ...
    \item[$z_i, \; i\in \{1,2,...,n\}$] is the location of the $i$-th leak downstream along the pipe such that $z_i < z_{i+1} \;\forall i$. Defining $z_0=0$ and $z_{n+1}=L$ simplifies equations.

    \item[$\tilde{q}_i$] is the flow leaking out of the $i$-th leak.

    \item[$h_i:=h(z_i)$] is the head at leak location $z_i$. $h_0$ and $h_L:=h_{n+1}$ are defined as the head at the inlet and outlet of the pipe.

    \item[$q_i:=q(z'), \; z'\in(z_i,z_{i+1})$] is the flow of the non-leaking pipe section after the $i$-th leak. $q_0$ and $q_L:=q_n$ are defined as the inlet and outlet flow of the pipe.
