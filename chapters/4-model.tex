...
A leak in the pipe is assumed to be modeled as a valve with a constant opening. Following \cite[pp.~54]{chaudhry2013}, the flow through a valve, denoted by $\tilde{q}$, at location $z^*$,can be expressed as
\begin{equation*}
...
In this section, the notation for a pipe with $n$ leaks is defined. \autoref{fig:leak_notation} illustrates the variables and indexing used to describe the system.
\begin{figure}[h!]
    \centering
...
\begin{description}[itemsep=2pt, topsep=0pt, parsep=0pt]
    \item[$\ell$] is the length of the pipe.
    
    \item[$z \in \lbrack 0,\ell \rbrack$] is the spatial coordinate along the pipe.
    
    \item[$z_i, \; i\in \{1,2,\dots,n\}$] is the location of the $i$-th leak downstream along the pipe such that $z_i < z_{i+1} \;\forall i$. Defining $z_0=0$ and $z_{n+1}=\ell$ simplifies equations.

    \item[$\tilde{q}_i$] is the flow leaking out of the $i$-th leak.

    \item[$h_i:=h(z_i)$] is the head at leak location $z_i$. $h_0$ and $h_{\ell}:=h_{n+1}$ are defined as the head at the inlet and outlet of the pipe.

    \item[$q_i:=q(z'), \; z'\in(z_i,z_{i+1})$] is the flow of the non-leaking pipe section after the $i$-th leak. $q_0$ and $q_{\ell}:=q_n$ are defined as the inlet and outlet flow of the pipe.

    \item[$L_i:=z_i-z_{i-1}$] is the length of the non-leaking pipe section between $z_{i-1}$ and $z_i$. Defining $L_{n+1}:=\ell-z_n$ yields $\sum_{k=1}^{n+1} L_k = \ell$.

    \item[$\mu:=\{h_0,h_{\ell},q_0,q_{\ell}\}$] is the set containing the head and flow at the inlet and outlet of the pipe, which can be measured.

    \item[$\mu^{(j)}:=\{h_0^{(j)},h_{\ell}^{(j)},q_0^{(j)},q_{\ell}^{(j)}\},\;j\in\{1,2,\dots,m\}$] is the $j$-th measurement and $m$ is the total number of measurements.
...
\end{equation}
and the head and flow are no longer time varying. Combining \cref{eq:momentum,eq:continuity} with \cref{eq:zero_derivatives} gives the steady state equations for a non-leaking pipe segment:
...
Given a pipe operating in steady state, and under the assumptions previously stated, the following equations and \autoref{fig:leak_notation_summary} describe the system around a leak at location $z_i$:
\begin{enumerate}
    ...
    \begin{align}
        q_{i} &= q_{i-1}-\tilde{q}_{i} \notag \\
        &= q_{i-1} - c_i\sqrt{h_i} \label{eq:upstream_flow}
    \end{align}
\end{enumerate}
