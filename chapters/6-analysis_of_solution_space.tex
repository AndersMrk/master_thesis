...
In \autoref{sec:der_jacobian}, an analytic expression for the Jacobian, $J(x)$, is derived. The Jacobian enables efficient analysis of the space around the solution. Recall from \cref{eq:jac_res_approx} that $J(x)$ approximates the residual locally around $x$. The condition number of the Jacobian, $\kappa(J(x))$, defined in \cref{eq:cond_num_def}, serves as a measure of how the residual behaves near $x$.\\

Let $\kappa^*:=\kappa(J(\mathbf{x}^*))$ denote the condition number of the Jacobian at the solution. A large $\kappa^*$ indicates that some directions in the residual space are more sensitive than others, suggesting that the corresponding solution is more difficult to identify reliably.\\

This chapter first investigates how the choice of measurements, $\mathcal{M}$, affects $\kappa^*$ and how to select measurements to minimize it. The chapter then examines how the condition number changes with the number of leaks and discusses how many leaks can feasibly be identified.

\section{Constructing Optimal Measurements}

This section will aim to construct measurements, $\mathcal{M}$, that minimizes $\kappa^*$, 
\begin{equation*}
    \underset{\mathcal{M}}{\min} \ \kappa^*,
\end{equation*}
which in this sense will be considered optimal. The approach is based on the knowledge that increasing the linear independence of the residuals improves the conditioning of the Jacobian.\\
...
\begin{figure}[ht]
    ...
    \caption[Condition number at the solution for different number of measurements]{Condition number of the Jacobian at the solution for different measurement matrices constructed by \autoref{alg:generate_measurements} and varying the number of measurements used}
    \label{fig:plt_m_cond}
\end{figure}

\subsection{Summary}

In this section, the condition number at the solution of leak-identification problems has been investigated across a variety of measurement matrices. Although only the limited case of evenly spaced leaks was considered, the results should generalize to other leak configurations. It was seen that increasing the range of inlet flows, $q_0$, in a measurement matrix, is equivalent to increasing the range of pressure drops, $\Delta h$. In practice, varying the inlet flow might be the most feasible, but for simulation purposes, \autoref{alg:generate_measurements} was designed to generate a measurement matrix with varying pressure drops.\\

Based on the results in this section, \autoref{alg:generate_measurements} can be considered an optimal method for generating a measurement matrix. Performance increases by increasing $R_{\Delta h}$ within the bounds of feasible operating points and by increasing the number of measurements. Increasing the number of measurements showed diminishing returns, and there is little effect from using more than $2n$ measurements.
