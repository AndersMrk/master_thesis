...
\begin{tabular}{@{}lr@{}}
    ...
    $r_k = r_y(y)$ & (\ref{eq:residual} \ref{eq:ry})\\
...
% Max iter reached
\node[block, right=17mm of d2] (retF) {No valid step found};
\draw[line] (d2.east) -- node[above,pos=0.2]{False} (retF.west);

% Inner loop body
\node[block, below=of d2] (inner_compute){\begin{tabular}{@{}lr@{}}
    Compute $s_k$ & (\ref{eq:sk_lm_solve})\\
    $y_{\text{trial}}=y_k+s_k$ & (\ref{eq:step_update_rule})\\
    $r_{\text{trial}}=r_y(y_{\text{trial}})$ & (\ref{eq:residual} \ref{eq:ry})\\
