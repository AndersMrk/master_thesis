...
\begin{tabular}{@{}lr@{}}
    $\bm{\xi}_k = g^{-1}(x_0)$ & (\ref*{eq:g_func})\\
    $r_k = r(g(\bm{\xi}))$ & (\ref*{eq:residual})\\
    $J_k = J_\xi(\bm{\xi})$ & (\ref*{eq:jacobian} \ref*{eq:Jy}) \\
...
{$\lVert r_k\rVert > \mathrm{\tau_\text{conv}}$ \\ $k < k_{\max}$};
\draw[line] (b0) -- node[right,pos=0.7]{$k=0$} (d1);

% Return x
\node[block, right=12mm of d1] (ret) {$x=g(\bm{\xi}_k)$,\ return $x$};
...
\begin{tabular}{@{}lr@{}}
    ...
    $\bm{\xi}_{\text{trial}}=\bm{\xi}_k+s_k$ & (\ref*{eq:step_update_rule})\\
    $r_{\text{trial}}=r(g(\bm{\xi}_{\text{trial}}))$ & (\ref*{eq:residual})\\
    $J_{\text{trial}}=J_\xi(\bm{\xi}_{\text{trial}})$ & (\ref*{eq:jacobian} \ref*{eq:Jy})\\
...
$\bm{\xi}_k = \bm{\xi}_{\text{trial}},\ r_k=r_{\text{trial}},\ J_k=J_{\text{trial}}$};
