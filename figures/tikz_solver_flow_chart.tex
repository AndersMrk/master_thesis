...
\begin{tabular}{@{}lr@{}}
    $\xi_k = g^{-1}(x_0)$ & (\ref*{eq:g_func})\\
    $r_k = r(g(\xi))$ & (\ref*{eq:residual})\\
    $J_k = J_\xi(\xi)$ & (\ref*{eq:jacobian} \ref*{eq:Jy}) \\
...
% Return x
\node[block, right=12mm of d1] (ret) {$x=g(\xi_k)$,\ return $x$};
...
\begin{tabular}{@{}lr@{}}
    ...
    $\xi_{\text{trial}}=\xi_k+s_k$ & (\ref*{eq:step_update_rule})\\
    $r_{\text{trial}}=r(g(\xi_{\text{trial}}))$ & (\ref*{eq:residual})\\
    $J_{\text{trial}}=J_\xi(\xi_{\text{trial}})$ & (\ref*{eq:jacobian} \ref*{eq:Jy})\\
...
$\xi_k = \xi_{\text{trial}},\ r_k=r_{\text{trial}},\ J_k=J_{\text{trial}}$};
